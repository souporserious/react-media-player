    ...
    _events() {
        return {
            ...
            onStateChange: ({ data }) => {
                ...
                //to avoid player to start from 0 on replay when playerVars set                
                const { config } = this.props;
                let isCustomised = Object.keys(config).length > 0 && (config.start || config.end);
                if (isPlaying) {
                    ...
                    if (isCustomised) {
                        ...
                        if (currentTime < config.start || currentTime > config.end) {
                            this._player.seekTo(config.start);
                        }
                    }
                } else {
                    cancelAnimationFrame(this._timeUpdateId)
